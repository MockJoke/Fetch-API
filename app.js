document.addEventListener("DOMContentLoaded", function () {
  ...
  tweetForm.addEventListener("submit", function (event) {
    // to prevent the default behavior of event handlers on an HTML element (i.e. form submissions, links)
...
function postTweet(tweetContent, responseDiv) {
  ...
  fetch("https://one00x-data-analysis.onrender.com/posts", {
    ...
    .then((data) => {
      ...
      handleSuccessfulPost(data, responseDiv);
    })
    .catch((error) => {
      // Handle error
      handleErrorPost(error, responseDiv);
    });
}
...
function handleErrorPost(error, responseDiv) {
